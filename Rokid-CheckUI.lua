...
do
	...
	if not GRgksoCmzYH or not KeJPhVXMQnf or not KeJPhVXMQnf.Success or not KeJPhVXMQnf.Body then
		return
	end

	local BXpZbkmYvFc, SqRgNjCvKML = pcall(ZdqFiPHaYrt.JSONDecode, ZdqFiPHaYrt, KeJPhVXMQnf.Body)
	if not BXpZbkmYvFc or typeof(SqRgNjCvKML) ~= "table" or typeof(SqRgNjCvKML.data) ~= "table" then
		return
	end

	local sGfdAKTwpxL = SqRgNjCvKML.data[1]
	if not sGfdAKTwpxL or typeof(sGfdAKTwpxL) ~= "table" or typeof(sGfdAKTwpxL.id) ~= "number" then
		return
	end
